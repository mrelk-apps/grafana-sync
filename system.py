...
def create_backup(type, url, cookies=""):
    ...
    Params: 
     ...
     - cookies: optional striing that matches your session cookies, can be used instead of username and password
    """
    ...
    backup_folder = "backup\\"+type+"\\"+str(now)+"\\"
    ...
    if create_folder(backup_folder) == "Created":
        for get_each in get_all.json():
            ...
            create_file(backup_folder, get_each["uid"]+".json", json.dumps(get_each))
...
def update_all(type, old_value, new_value, url, cookies=""):
    ...
    now = datetime.now().strftime("%Y%m%d%H%M%S")
    headers = {'Cookie': cookies}
    print("Please wait while we update the values mentioned and save updated json files for all your "+type+"...")
    api_uid = "/api/"+type+"/uid/"
    update_folder = "updated\\"+type+"\\"+str(now)+"\\"
    ...
    if create_folder(update_folder) == "Created":
        ...
        print("Completed, Check folder ["+update_folder+"].")
    else:
        print("Coudlnt create the folder, please try again")
...
def search_all(type, value, url, cookies=""):
    ...
    now = datetime.now().strftime("%Y%m%d%H%M%S")
    headers = {'Cookie': cookies}
    print("Please wait while we search the value mentioned and save result json files for all your "+type+"...")
    api_uid = "/api/"+type+"/uid/"
    search_folder = "search_results\\"+type+"\\"+str(now)+"\\"
    if type == "datasources":
        api_all =  "/api/datasources"
    elif type == "dashboards":
        api_all =  "/api/search?query=%"
    ...
    if create_folder(search_folder) == "Created":
        ...
        print("Completed, Check folder ["+search_folder+"].")
    else:
        print("Coudlnt create the folder, please try again")
